...
# TODO
#argv$source<-'sim_data/ch3/balance_stdp/unimodal/'
...
# TODO remove, kept just in case
#df_weights <- read_feather(file.path(data_path, 'obj_vars.feather'), as_data_frame=F)
#write_parquet(df_weights, file.path(data_path, 'obj_vars.parquet'))
#rm(df_weights)
#gc()

# TODO one from and to neuron
#ds_weights <- open_dataset(file.path(data_path, 'obj_vars.feather'), format="arrow")
#w_hist <- hist((ds_weights %>% select(w_plast) %>% collect())$w_plast, plot=F)
#df_weights <- data.frame(count = w_hist$counts,
#                         mids = w_hist$mids)
#df_weights$mids <- df_weights$mids * 1000
#
#w_distribution <- df_weights %>%
#    ggplot(aes(x=mids, y=count)) +
#    geom_col(width=.019) + theme_bw() +
#    labs(x='weights (mV)')
## TODO color
##    #scale_color_manual(values=color_map[2]) +
##    #scale_fill_manual(values=color_map[2])
#ggsave(str_replace(argv$dest, '.png', '_weight.png'), w_distribution)
#gc()
